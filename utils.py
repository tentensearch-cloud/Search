...
TARGET_COMPANIES_STR = "Anduril,スカパーJSAT,GSユアサ,ミネベアミツミ,ニコン,マクニカHD,伊藤忠,NEC,住友商事,信越化学工業,KDDI,古野電気,ハーモニック・ドライブ・システムズ,ACSL,レンゾ,三菱重工業,ニデック,TDK,双葉電子工業,浜松ホトニクス,東レ,ソニー,アスター,航空電子工業,エクセディ,ジェイテクト"
...
取得したニュース、IR情報をポジティブかネガティブか判定した上で、ポジティブ要素とネガティブ要素それぞれ3行程度で簡潔に要約してください。
そしてその結果から({TARGET_COMPANIES_STR})は買い目であるか売り目であるか判断し理由は記載せず、企業名と買い目であるか売り目であるかを記載してください。
...
回答にロールプレイに関する返答を含めないで下さい。
回答の先頭はニュースのタイトルにしてください、【報告】と同様の回答が表示される為。
ニュースの内容は日本語で翻訳の必要がない時は翻訳はスキップしてください。
ニュース、IR情報が日本語ではない時は日本語に翻訳してから要約を行ってください。
